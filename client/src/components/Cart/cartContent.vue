<template>
  <div class="cartContent bg_grey p_10">
    <div class="userCart p_10">
      <div class="userCart--total color_black p_10 t_bold">
        Sản phẩm bạn đã chọn ({{ products.length }})
      </div>
      <div class="userCart--list mt_10">
        <div
          class="userCart--list-item"
        ...
        >
          <div class="infoProduct">
            ...
            <div class="infoProduct--detail">
              <h5 class="infoProduct--detail-name">
                {{ product.product_name }}
              </h5>
              <span class="infoProduct--detail-price t_bold color_red"
                >{{ product.product_price }}đ</span
              >
            </div>
            <div class="infoProduct--action t_right"><button>Xóa</button></div>
          </div>
          <div class="quantilyProduct mt_10">
            <div class="quantilyProduct--status color_black">
              <i class="icon fas fa-circle"></i>Sản phẩm khả dụng
            </div>
            <div class="quantilyProduct--number">
              <i
                class="icon fas fa-minus-circle"
                :class="{
                  'icon-disable': product.product_quantity <= 0
                }"
                @click="dec(index);"
              ></i>
              <input
              ...
              />
              <i class="icon fas fa-plus-circle" @click="inc(index);"></i>
            </div>
          </div>
        </div>
      </div>
...
export default {
    ...
    return {
      products: [
        ...
        },
        {
          id: 2,
          product_image: "../../assets/images/slider-bg.png",
          product_name: "Mì xào cay cấp độ n + 3",
          product_price: 2000000000000000,
          product_quantity: 1
        }
      ]
    };
  ...
  methods: {
    inc(index) {
      if (this.products[index].product_quantity < 10)
        return this.products[index].product_quantity++;
      else alert("an cai deu gi nua!");
    },
    dec(index) {
      return this.products[index].product_quantity--;
    }
